"""
Independent Diffusion for RSBench.

This script runs the independent diffusion model - same as NeSyDM
but with factorized/independent predictions per concept.

Usage:
    python indepdiffusion.py --dataset halfmnist --backbone partialentangled ...
"""
import argparse
import os
import time
...
from expressive.experiments.rsbench.models.indep_diffusion import create_indep_diffusion
...
if __name__ == "__main__":
    ...
    run = wandb.init(
        project=f"nesy-indep-diffusion-rsbench",  # Different project name to distinguish
        tags=["independent-diffusion"],
    ...
    # Use independent diffusion model instead of regular NeSyDM
    model = create_indep_diffusion(args, dataset).to(device)
